...
function Jira({ className, templates, index, name, remove, update, fieldsLength, ...rest }: Props) {
  ...
  React.useEffect(() => {
    ...
    setValue(
      `${name}.${ASSISTANTS_FIELDS.SETTINGS}.${JIRA_FIELDS.ACCESS_TOKEN}`,
      settings.access_token
    );
    setValue(`${name}.${ASSISTANTS_FIELDS.SETTINGS}.${JIRA_FIELDS.BASE_URL}`, settings.base_url);
  }, [settings, setValue]);
  ...
  const handleDelete = () => {
    if (id) {
      ...
      setValue(`${name}.${ASSISTANTS_FIELDS.SETTINGS}.${JIRA_FIELDS.ACCESS_TOKEN}`, '');
      setValue(`${name}.${ASSISTANTS_FIELDS.SETTINGS}.${JIRA_FIELDS.BASE_URL}`, '');
      remove(index);
    }
  };
  return (
    <Flex direction="column" gap="md">
      <Flex className={cx(classes.root, className)} gap="md">
        ...
        <InputField
          ...
          name={`${name}.${ASSISTANTS_FIELDS.SETTINGS}.${JIRA_FIELDS.ACCESS_TOKEN}`}
          label="Токен"
          placeholder="Пример: 1112223334"
        />
        <InputField
          className={classes.input}
          name={`${name}.${ASSISTANTS_FIELDS.SETTINGS}.${JIRA_FIELDS.BASE_URL}`}
          label="Адрес сервера Jira"
          placeholder="Пример: 1112223334"
        />
      </Flex>
    </Flex>
